...
void countingSort(int input_array[],int s, int r){
	...
	// posiÁıes de elementos a serem armazenados na matriz de saÌda
	for(int i=1;i<r;i++)
		count_array[i]=count_array[i]+count_array[i-1];
	
	// colocando os elementos do array de entrada no array de saÌa com as
	//  posiÁıes de modo que o resultado seja um array classificado na ordem ASC
	for(int i=0;i<s;i++)
		output_array[--count_array[input_array[i]]] = input_array[i];
	
	// copiar elementos da matriz de saÌda para a matriz de entrada
	for(int i=0;i<s;i++)
...
void heapify(int arr[], int n, int i){
	...
	// Se o filho certo for maior que o maior atÈ agora
...
void heapSort(int arr[], int n){
	...
	for (int i = n - 1; i >= 0; i--){
		// Move raiz atual pra √∫ltima folha
		swap(arr[0], arr[i]);
...
void merge(int *array, int l, int m, int r) {
   ...
   for(j = 0; j<nr; j++)
      rarr[j] = array[m+1+j];
   i = 0; j = 0; k = l;
   //merge arrays tempor√°rios em array real
   while(i < nl && j<nr) {
      if(larr[i] <= rarr[j]) {
         array[k] = larr[i];
         i++;
...
int partition(int arr[], int start, int end){
	...
	// Dando ao elemento piv√¥ sua posiÁ„o correta
	int pivotIndex = start + count;
	swap(arr[pivotIndex], arr[start]);

	// Classificando as partes esquerda e direita do elemento pivÙ
	int i = start, j = end;
...
void quickSort(int arr[], int start, int end){
	...
	// ordena a parti√ß√£o esquerda
	quickSort(arr, start, p - 1);

	// ordena a parti√ß√£o direita
...
int main(){
	...
	cout<<"-------OrdenaÁ„o com array ordenado crescente-------\n";
    cout<<"-------OrdenaÁ„o por Count Sort-------\n";
    ...
    cout<<"-------OrdenaÁ„o com array ordenado decrescente-------\n";
    cout<<"-------OrdenaÁ„o por Count Sort-------\n";
	...
	cout<<"-------OrdenaÁ„o com array ordenado aleatoriamente-------\n";
    cout<<"-------OrdenaÁ„o por Count Sort-------\n";
